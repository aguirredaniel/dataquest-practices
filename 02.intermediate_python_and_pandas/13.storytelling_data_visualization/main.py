...
def main():
    ...
    cumulative_deaths = [2398, 126203, 227178, 295406]
    ...
    for ax, period, period_label_params, deaths in zip(axes, periods, periods_labels_params, cumulative_deaths):
        ...
        print(death_toll['New_deaths'][start:finish].sum())
        # Adding Progress bars
        ax.axhline(y=1600, xmin=0.5, xmax=0.8,
                   linewidth=6, color='#af0b1e', alpha=0.1)
        # Calculating proportion of deaths
        proportion = round(deaths / 295406, 2)
        xmax_val = round(0.5 + proportion * 0.3, 3)

        # Showing proportion result
        ax.text(7.5, 1850, format(deaths, ','), color='#af0b1e', weight='bold')
        ax.axhline(y=1600, xmin=0.5, xmax=xmax_val,
